<!DOCTYPE html>
<html>
<head>
    <link rel="stylesheet" href="../css/style.css"> <!-- CSS 스타일 연결 -->
</head>
<body>
    <header>
        <div class="header-container">
            <a href="/" class="logo"><img src="https://munit.me/image/Group 18.png" alt="Logo" class="logoimg"></a>
            <nav>
                <ul id="topMenu">
                    <li><a href="#">DUT²소개</a>
                        <ul class="sub-menu leftbox">
                            <li><a href="#">둣둣 소개</a></li>
                            <li><a href="#">오픈소스</a></li>
                            <li><a href="#">둣둣개발팀</a></li>
                        </ul>
                    </li>
                    <li><a href="#">코드</a>
                        <ul class="sub-menu leftbox">
                            <li><a href="/sharecode">코드 공유</a></li>
                            <li><a href="#">코드 질문</a></li>
                        </ul>
                    </li>
                    <li><a href="#">찾아보기</a>
                        <ul class="sub-menu leftbox">
                            <li><a href="#">유저찾기</a></li>
                        </ul>
                    </li>
                    <li><a href="#">커뮤니티</a>
                        <ul class="sub-menu leftbox">
                            <li><a href="#">엔트리스토리</a></li>
                            <li><a href="#">둣교일지</a></li>
                            <li><a href="#">공지사항</a></li>
                        </ul>
                    </li>
                </ul>
            </nav>
            <a href="/login" class="loginButton" id="logbt">로그인</a>
            <div id="menuTogle">
                <img class="tog" src="https://munit.me/image/hamburger.svg">
            </div>
        </div>
    </header>
    <!-- 이전에 여기 있던 code.js 스크립트 태그 제거 -->
    <script src="https://munit.me/mobile.js" defer></script>
</body>
</html>


